...
from schemas import Mensagem, UsuarioSchema, UsuarioBD, UsuarioPublic
...
def teste(
    numero: Annotated[int, 
            Query(title="Número", 
            description="Um número inteiro", ge=1, le=10)] = 5
    ):
...
def buscar_usuario_pelo_id(id: int):
    ...
    return {"msg": f"Usuário com ID {id}"}
...
@app.post("/usuarios", response_model=UsuarioPublic)
